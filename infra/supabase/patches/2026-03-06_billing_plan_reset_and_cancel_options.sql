...
-- =========================================================
-- 1) Comprar/cambiar plan = solo deja los créditos del plan nuevo
--    Conserva topups, pero resetea buckets ligados al plan:
--    - gen_plan_credits
--    - gen_bonus_credits
-- =========================================================
...
-- =========================================================
-- 2) Cancelación configurable:
--    - sin borrar créditos
--    - borrando créditos de generación
-- =========================================================

create or replace function public.billing_cancel_subscription(
  p_user_id uuid,
  p_wipe_generation_credits boolean default false,
  p_idempotency_key text default null
) returns table (
  wipe_generation_credits boolean,
...
begin
  ...
  if p_idempotency_key is not null and length(trim(p_idempotency_key)) > 0 then
    ...
    if v_ledger.id is not null then
      ...
      return query
      select
        coalesce((v_ledger.meta ->> 'wipe_generation_credits')::boolean, false),
        coalesce(v_bal.gen_plan_credits, 0),
        coalesce(v_bal.gen_topup_credits, 0),
        coalesce(v_bal.gen_bonus_credits, 0);
      return;
  ...
  if p_wipe_generation_credits then
    update public.wallet_balances
  ...
  insert into public.wallet_ledger(
    ...
    p_user_id,
    case
      when p_wipe_generation_credits then 'cancel_subscription_wipe_generation'
      else 'cancel_subscription_keep_generation'
    end,
    0,
    ...
    p_idempotency_key,
    jsonb_build_object(
      'wipe_generation_credits', p_wipe_generation_credits,
      'before_plan_credits', v_before_plan,
      'before_topup_credits', v_before_topup,
      'before_bonus_credits', v_before_bonus,
      'after_plan_credits', coalesce(v_bal.gen_plan_credits, 0),
...
revoke all on function public.billing_cancel_subscription(uuid, boolean, text) from PUBLIC, anon, authenticated;
grant execute on function public.billing_cancel_subscription(uuid, boolean, text) to service_role;
