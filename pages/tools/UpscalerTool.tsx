...
type PanelKey = "reference" | "model" | "parameters" | null;
...
const UpscalerTool: React.FC<{ prefillAsset?: Asset | null }> = ({ prefillAsset }) => {
  ...
  const filteredPickerAssets = useMemo(() => {
    ...
    return pickerAssets.filter((asset) => `${asset.name || ""} ${removeUpscaleMasterBlock(asset.prompt || "") || ""}`.toLowerCase().includes(q));
  ...
  return (
    <div ref={rootRef} className={`${styles.root} ${isCookOpen ? styles.rootCookOpen : ""}`}>
      ...
      {panel === null && (
        <button type="button" className={`${styles.cookToggle} ${isCookSidebarVisible ? styles.cookToggleOpen : styles.cookTogglePulse}`} onClick={() => { if (isCookSidebarVisible) closeCook(); else openCook(); }} aria-expanded={isCookSidebarVisible} aria-controls="upscaler-start-create" aria-label={isCookSidebarVisible ? "Close Start Create" : "Open Start Create"}>
      ...
      {isCookLayerVisible && (
        <div id="upscaler-start-create" className={`${styles.cookOverlay} ${styles.cookOverlayOpen}`} aria-hidden={!isCookLayerVisible}>
          <button type="button" className={styles.cookBackdrop} aria-label={panel ? "Back to Start Create" : "Close Start Create"} tabIndex={isCookLayerVisible ? 0 : -1} onClick={() => { if (panel) restoreCookFromPanel(); else closeCook(); }} />
          {panel && (
            <div className={`${styles.cookPanelShell} ${!isCookSidebarVisible ? styles.cookPanelShellSolo : ""}`}>
              <div ref={popoverRef} className={styles.cookPanel}>
                {panel === "reference" && (<>
                  <div className={styles.popoverHeader}><div className={styles.popoverTitle}>Reference</div><div className={styles.headerRight}><div className={styles.cookSectionMeta}>{referenceLabel}</div><button type="button" className={styles.closeBtn} onClick={restoreCookFromPanel} aria-label="Close Reference">×</button></div></div>
                  <div className={`${styles.popoverBody} ${styles.cookPanelBody} ${styles.cookPanelScroll}`}>
                    <div className={styles.refSlot}>
                      <div className={styles.refSlotLeft}><div className={styles.refSlotLabel}>Reference Image</div>{baseRef ? <div className={styles.refSlotThumb}><img src={baseRef.url} alt={baseRef.name || "Reference"} /></div> : <div className={`${styles.refSlotThumb} ${styles.refSlotThumbEmpty}`}>+</div>}</div>
                    ...
                    <div className={styles.pickerGrid}>{filteredPickerAssets.map((asset) => (<button key={asset.id} type="button" className={styles.pickerTile} onClick={() => { setBaseRef(asset); restoreCookFromPanel(); }}><img src={asset.url} alt={asset.name} /><div className={styles.pickerTileMeta}><div className={styles.pickerTileCap}>{removeUpscaleMasterBlock(asset.prompt || "") || asset.name}</div></div></button>))}{filteredPickerAssets.length === 0 ? <div className={styles.pickerEmpty}>No images found in your library.</div> : null}</div>
                  </div>
                </>)}
                {panel === "model" && (<>
                  <div className={styles.popoverHeader}><div className={styles.popoverTitle}>Model</div><div className={styles.headerRight}><div className={styles.cookSectionMeta}>{modelLabel}</div><button type="button" className={styles.closeBtn} onClick={restoreCookFromPanel} aria-label="Close Model">×</button></div></div>
                  <div className={`${styles.popoverBody} ${styles.cookPanelBody} ${styles.cookPanelScroll}`}><div className={styles.modelGrid}><div className={styles.modelGroup}><div className={styles.modelGroupLabel}>Available</div><div className={styles.modelGroupOptions}>{MODEL_OPTIONS.map((opt) => (<button key={opt.id} type="button" className={`${styles.modelOption} ${model === opt.id ? styles.modelOptionActive : ""}`} onClick={() => { setModel(opt.id); restoreCookFromPanel(); }}>{opt.label}</button>))}</div></div></div></div>
                </>)}
                {panel === "parameters" && (<>
                  <div className={styles.popoverHeader}><div className={styles.popoverTitle}>Parameters</div><div className={styles.headerRight}><div className={styles.cookSectionMeta}>{parametersLabel}</div><button type="button" className={styles.closeBtn} onClick={restoreCookFromPanel} aria-label="Close Parameters">×</button></div></div>
                  <div className={`${styles.popoverBody} ${styles.cookPanelBody} ${styles.cookPanelScroll}`}><div className={styles.paramGrid}><div className={styles.formRow}><label className={styles.formLabel}>Quality</label><select className={styles.select} value={qualityLabel} onChange={(e) => { setQuality(e.target.value as Quality); restoreCookFromPanel(); }}>{allowedQualities.map((q) => (<option key={q} value={q}>{q}</option>))}</select></div></div></div>
                </>)}
              </div>
            </div>
          )}
