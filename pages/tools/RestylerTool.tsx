...
type PanelKey = "reference" | "style" | "model" | "parameters" | null;
...
const RestylerTool: React.FC = () => {
  ...
  const filteredPickerAssets = useMemo(() => {
    ...
    return pickerAssets.filter((asset) => `${asset.name || ""} ${removeStylePresetBlock(asset.prompt || "") || ""}`.toLowerCase().includes(q));
  }, [pickerAssets, pickerQuery]);

  const filteredStylePresets = useMemo(() => {
    const q = (pickerQuery || "").trim().toLowerCase();
    if (!q) return STYLE_PRESETS;
    return STYLE_PRESETS.filter((preset) => String(preset.name || "").toLowerCase().includes(q));
  }, [pickerQuery]);
    ...
    <div ref={rootRef} className={`${styles.root} ${isCookOpen ? styles.rootCookOpen : ""}`}>
      ...
      {panel === null && (
        <button
          ...
          aria-controls="restyler-start-create"
            ...
            <div className={`${styles.cookPanelShell} ${!isCookSidebarVisible ? styles.cookPanelShellSolo : ""}`}>
              <div ref={popoverRef} className={styles.cookPanel}>
                        ...
                        <div className={styles.refSlotActions}>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (!file) return;
                              void uploadReference(file).then((asset) => {
                                setBaseRef(asset);
                                restoreCookFromPanel();
                              }).catch((err: any) => setError(err?.message || "Upload falló."));
                              e.currentTarget.value = "";
                            }}
                          />
                          <button type="button" className={styles.smallBtn} onClick={() => refFileInputRef.current?.click()}>Upload</button>
                          {baseRef && (
                            <button type="button" className={styles.smallBtnGhost} onClick={() => setBaseRef(null)}>Remove</button>
                          )}
                        </div>
                      </div>
                ...
                {panel === "style" && (
                  <>
                    <div className={styles.popoverHeader}>
                      <div className={styles.popoverTitle}>Style</div>
                      <div className={styles.headerRight}>
                        <div className={styles.cookSectionMeta}>{styleLabel}</div>
                        <button type="button" className={styles.smallBtnGhost} onClick={() => { setSelectedStyleId(null); restoreCookFromPanel(); }}>Clear</button>
                        <button type="button" className={styles.closeBtn} onClick={restoreCookFromPanel} aria-label="Close Style">×</button>
                      </div>
                    </div>
                    <div className={`${styles.popoverBody} ${styles.cookPanelBody} ${styles.cookPanelScroll}`}>
                      <input
                      ...
                      <div className={styles.presetGrid}>
                        ...
                        {filteredStylePresets.length === 0 ? <div className={styles.pickerEmpty}>No hay estilos que coincidan.</div> : null}
                      </div>
                    </div>
                  </>
                )}
