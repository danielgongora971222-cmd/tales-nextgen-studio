...
// Separate block markers to avoid mixing STYLE presets with LIGHTING presets.
const LIGHTING_PRESET_BLOCK_START = "[[LIGHTING_PRESET_START]]";
const LIGHTING_PRESET_BLOCK_END = "[[LIGHTING_PRESET_END]]";

// Keep legacy STYLE markers so we can strip them if they appear (prevents mixing).
...
function removeLightingPresetBlock(input: string) {
  let out = input;

  // Remove BOTH blocks to ensure STYLE + LIGHTING never mix.
  const pairs = [
...
function applyLightingPresetToPrompt(input: string, presetPrompt: string) {
  const base = removeLightingPresetBlock(input).trim();
  const block = `${LIGHTING_PRESET_BLOCK_START}\n${presetPrompt}\n${LIGHTING_PRESET_BLOCK_END}\n\n`;
  return `${block}${base}`.trim();
}

function extractLightingBlock(input: string) {
  const pairs = [{ start: LIGHTING_PRESET_BLOCK_START, end: LIGHTING_PRESET_BLOCK_END }];
...
function getLightingNameFromPrompt(prompt: string): string {
  const inside = extractLightingBlock(prompt || "");
...
function LightingPickerModal(props: {
  open: boolean;
  ...
  const { open, selectedLightingId, onSelect, onClear, onClose } = props;
  const [query, setQuery] = useState("");

  const filtered = useMemo(() => {
    const q = (query || "").trim().toLowerCase();
    if (!q) return LIGHTING_PRESETS;
    return LIGHTING_PRESETS.filter((p) => String(p.name || "").toLowerCase().includes(q));
  }, [query]);
    ...
    <div className={videoStyles.modalOverlay} role="dialog" aria-modal="true">
      <div className={videoStyles.modal}>
        <div className={videoStyles.modalHeader}>
          <div className={videoStyles.modalTitle}>Select lighting</div>
        ...
        <div className={videoStyles.modalActions}>
          <input
            ...
            placeholder="Buscar iluminaciones…"
            value={query}
        ...
        <div className={styles.presetGridFixed}>
          ...
          {filtered.length === 0 && <div className={videoStyles.pickerEmpty}>No hay iluminaciones que coincidan.</div>}
        </div>
      </div>
    </div>
  );
...
const LightroomTool: React.FC = () => {
  ...
  async function handleGenerate() {
    ...
    if (!selectedLighting) {
      setError("Selecciona una Iluminación.");
      return;
    }
          ...
          {history.length === 0 && pendingSlots.length === 0 ? (
            <div className={styles.emptyState}>
              ...
              <div className={styles.emptyCopy}>
                ...
                <div className={styles.emptyText}>Elige una imagen + una iluminación y genera tu primer relight.</div>
              </div>
            </div>
          ) : (
            <div className={styles.grid}>
              ...
              {visibleHistory.map((asset) => {
                const caption = removeLightingPresetBlock(asset.prompt || "") || asset.name || "—";
                return (
                  <button
                    key={asset.id}
                    type="button"
              ...
              <div className={styles.viewerRecipe}>
                ...
                <div className={styles.recipeGrid}>
                  ...
                  <div className={styles.recipeItemWide}>
                    <div className={styles.recipeLabel}>Lighting</div>
                    <div className={styles.recipeValue}>{viewerRecipeInfo?.lightingName || "None"}</div>
                  </div>
                </div>
                ...
                <div className={styles.recipeBlock}>
                  ...
                  <div className={styles.recipeValue}>{removeLightingPresetBlock(viewer.prompt || "") || "—"}</div>
                </div>
              </div>
            </div>
          </div>
