...
<div className={styles.selectionRow}>
          {/* LEFT PANEL: 70% VISOR VISUAL CON INGENIERÍA DE MATERIALES */}
          <div className={styles.previewLeftPanel}>
            {selected ? (
              <div className={styles.previewCanvas}>
                <div className={`${styles.materialWrapper} ${styles[`material_${material}`]}`}>
                  <img className={styles.previewImg} src={selected.url} alt={selected.name || "Selected"} />
                  {/* Overlay físico interactivo según el concepto */}
                  {material === "metal" && <div className={styles.metalOverlay} />}
                </div>
                
                <div className={styles.previewMeta}>
                  <div className={styles.previewName}>{selected.name || "Untitled"}</div>
                  <div className={styles.previewSmall}>
                    <span className={styles.badge}>Herramienta: {selected.tool || "unknown"}</span>
                    <span className={styles.badge}>
                      {dimsLoading ? "Analizando píxeles..." : dims ? `${dims.w}×${dims.h}` : "Resolución desconocida"}
                    </span>
              ...
              <div className={styles.previewEmpty}>
                {/* Animación fluida de carga y espera */}
                <div className={styles.uploadIconBounce}>
                  <svg width="48" height="48" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                  </svg>
                </div>
                <div className={styles.previewEmptyCode}>SELECCIONA O ARRASTRA TU OBRA</div>
                <div className={styles.previewEmptyText}>Elige una imagen de tu historial para visualizarla en el entorno de estudio y aplicar acabados físicos.</div>
              </div>
            )}
          </div>

          {/* RIGHT PANEL: 30% CONTROLES PREMIUM */}
          <div className={styles.actionsRightPanel}>
            <div className={styles.panelSectionTitle}>1. Sube tu Obra</div>
            <button
              type="button"
            ...
            >
              {loadingAssets ? "Sincronizando..." : "Seleccionar desde Historial"}
            </button>

            <div className={styles.panelSectionTitle} style={{ marginTop: '24px' }}>2. Elige tu Material Base</div>
            <div className={styles.materialGrid}>
              <button type="button" className={`${styles.materialCard} ${material === "metal" ? styles.cardActive : ""}`} onClick={() => setMaterial("metal")}>
                ...
                <div className={styles.cardDesc}>Vibrante · Ultra brillante</div>
              </button>
              <button type="button" className={`${styles.materialCard} ${material === "acrylic" ? styles.cardActive : ""}`} onClick={() => setMaterial("acrylic")}>
                ...
                <div className={styles.cardDesc}>Profundidad · Premium</div>
              </button>
              <button type="button" className={`${styles.materialCard} ${material === "canvas" ? styles.cardActive : ""}`} onClick={() => setMaterial("canvas")}>
                ...
                <div className={styles.cardDesc}>Clásico · Textura artística</div>
              </button>
              <button type="button" className={`${styles.materialCard} ${material === "paper" ? styles.cardActive : ""}`} onClick={() => setMaterial("paper")}>
                ...
                <div className={styles.cardDesc}>Nítido · Tradicional</div>
              </button>
            </div>

            <div style={{ flexGrow: 1 }} /> {/* Espaciador flexible para empujar botones al fondo */}

            {blockReason && selected && (
              <div className={styles.blockBox}>
                <div className={styles.blockTitle}>Se requiere Atención</div>
            ...
            <button
              ...
              onClick={() => setStep(2)}
              title={!canContinue ? "Selecciona una imagen 4K para continuar." : "Continuar"}
            >
              Continuar a Medidas (Paso 2)
            </button>
          </div>
        </div>
...
const PrintStudio: React.FC<PrintStudioProps> = ({ step, setStep, selected, dims, ok4k, material }) => {
          ...
          {step === 2 && (
            <>
              <div className={styles.panelBlock}>
                <div className={styles.panelTitle}>Medidas Disponibles</div>
